...
function buildSlug(family, year, session, tz, paper) {
  // family: 'aa-hl' (new) sau 'math-hl' (old)
...
export default function PastPapersClient({ yearsNew = [], yearsOld = [] }) {
  ...
  // slug + href bazat pe curriculum
  const family = curr === "new" ? "aa-hl" : "math-hl";
  const baseHref =
    curr === "new" ? "/AA_HL/past-papers/" : "/MATH_HL/past-papers/";
  ...
  // culoarea închisă oficială AA HL (mov închis din globals)
  // tu ai folosit var(--aa-hl-dark) în CSS deja
  const DARK = "var(--aa-hl-dark)";
  ...
  return (
    <div className="rounded-3xl ring-2 ring-[#0f6a75]/10 mb-5">
      <div className="rounded-[calc(1.5rem-2px)] border border-slate-200 bg-white/60 shadow-sm">
        ...
        {/* grid sesiuni (carduri cu Paper 1 / Paper 2 / Paper 3) */}
                    ...
                    return (
                      <a
                        ...
                        className="
                          ...
                          transition
                          hover:bg-[var(--aa-hl-dark)]
                          hover:border-[var(--aa-hl-dark)]
                          hover:text-white
                        "
                      >
                        {`Paper ${p}`}
