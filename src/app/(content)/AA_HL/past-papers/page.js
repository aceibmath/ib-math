...
// Indexuri: new (AA HL, 2021–present) + old (Math HL, 2014–2020)
import { INDEX as AA_HL_INDEX } from "@/data/past-papers/aa-hl/index.js";
import { INDEX as MATH_HL_OLD_INDEX } from "@/data/past-papers/old/math-hl/index.js";

/* ---------- helpers pentru ani (same ca la AA SL/AI SL) ---------- */
...
export default function Page() {
  // New curriculum (AA HL, 2021–present)
  const yearsNew =
    extractYears(AA_HL_INDEX).filter((y) => y >= 2021) || rangeDesc(2025, 2021);
  const finalYearsNew =
    yearsNew.length > 0 ? yearsNew.sort((a, b) => b - a) : rangeDesc(2025, 2021);

  // Old curriculum (Math HL, 2014–2020)
  const yearsOld =
    extractYears(MATH_HL_OLD_INDEX).filter((y) => y <= 2020 && y >= 2014) || rangeDesc(2020, 2014);
  const finalYearsOld =
  ...
  return (
    <PageHeader>
      ...
      <PastPapersTitleCard course="AA HL" theme="aa-hl" />
