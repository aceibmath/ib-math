// src/app/(content)/AA_HL/teacher-lessons/_components/ThreePane.jsx
"use client";
...
const BG_ACTIVE  = "bg-[#F3E8FF]";     // mov pastel selectat (AA HL)
const TEAL_DARK  = "#7E22CE";          // mov închis (accent AA HL)
...
function LessonRow({ l, active, onSelect }) {
  return (
    ...
    >
      ...
      {active && (
        <div className="mt-2">
          {/* Butonul primește stilul violet din .course-theme-aa-hl definit în globals.css */}
          <Link
            href={`/AA_HL/teacher-lessons/lesson/${encodeURIComponent(l.id)}`}
...
export default function ThreePane({ topics }) {
  ...
  const [activeTopicKey, setActiveTopicKey] = useState(
    firstTopic?.key ?? "number-and-algebra"
  );
  const topic = useMemo(
  ...
  // 👇 Aici am adăugat clasa globală pentru tema AA HL
  return (
    <div className="course-theme-aa-hl mt-2 rounded-2xl bg-white border border-slate-200 p-3 md:p-4">
              ...
              {topics.map((t) => (
                <li key={t.key}>
                  <TopicItem
                    ...
                    onSelect={(k) => setActiveTopicKey(k)}
                  />
                </li>
              ))}
            </ul>
