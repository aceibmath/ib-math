// src/app/(content)/AI_HL/page.js
export const metadata = { title: "AI HL | AceIBMath" };

import CourseTitleCardAIHL from "@/components/course-titles/CourseTitleCard-ai-hl";
import styles from "./page.module.css";

export default async function Page({ searchParams }) {
  // Next 15: dynamic route params trebuie "await"-uite
  const sp = await searchParams;

  // aud=student | aud=teacher (default: student)
  const audience =
    (sp?.aud || "").toLowerCase() === "teacher" ? "teacher" : "student";

  // helper: atașăm audiența pe toate link-urile din grilă
  const withAud = (path) => `${path}?aud=${audience}`;

  // Tag unic (roșu) pe fiecare card
  const Tag = () => (
  ...
  const cards = [
    {
      href: withAud("/AI_HL/questionbank"),
      title: "Questionbank",
      teachersOnly: false,
      icon: (
        <svg
    ...
    {
      href: withAud("/AI_HL/past-papers"),
      title: "Past Papers",
      teachersOnly: false,
      icon: (
        <svg
    ...
    {
      href: withAud("/AI_HL/teacher-lessons"),
      title: "Teacher\nLessons",
      teachersOnly: true,
      icon: (
        <svg
    ...
    {
      href: withAud("/AI_HL/prediction-exams"),
      title: "Prediction\nExams",
      teachersOnly: false,
      icon: (
        <svg
    ...
    {
      href: withAud("/AI_HL/assessments"),
      title: "Assessments",
      teachersOnly: true,
      icon: (
        <svg
