// src/app/(content)/AI_HL/past-papers/page.js
export const metadata = { title: "AI HL Past Papers" };
...
import { INDEX as AI_HL_INDEX } from "@/data/past-papers/ai-hl/index.js";
import { INDEX as MATH_HL_OLD_INDEX } from "@/data/past-papers/old/math-hl/index.js";

/* ---------- helpers (identice cu cele din celelalte pagini) ---------- */
function uniq(nums) {
  return [...new Set(nums)];
}
  ...
  if (Array.isArray(idx)) {
    const arr = idx.flatMap((v) => {
      ...
      if (v && typeof v === "object") {
        return Object.keys(v).map((k) => Number(k)).filter(Number.isFinite);
      }
      return [];
    });
...
export default function Page() {
  // New curriculum (AI HL, 2021–present)
  const yearsNew =
    extractYears(AI_HL_INDEX).filter((y) => y >= 2021) || rangeDesc(2025, 2021);
  const finalYearsNew =
    yearsNew.length > 0 ? yearsNew.sort((a, b) => b - a) : rangeDesc(2025, 2021);

  // Old curriculum (Math HL, 2014–2020)
  const yearsOld =
    extractYears(MATH_HL_OLD_INDEX).filter((y) => y <= 2020 && y >= 2014) ||
    rangeDesc(2020, 2014);
  const finalYearsOld =
  ...
  return (
    <PageHeader>
      {/* Card titlu + descriere — identic cu AI SL */}
      <PastPapersTitleCard course="AI HL" theme="ai-hl"/>

      {/* Card probleme — fără margin custom, aliniat cu AI SL */}
      <section>
