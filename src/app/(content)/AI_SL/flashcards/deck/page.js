// src/app/(content)/AI_SL/flashcards/deck/page.js
"use client";
...
import ChoicesPanel from "@/components/flashcards/deck/ChoicesPanel-ai-sl";
import SubmitBar from "@/components/flashcards/deck/SubmitBar";
import styles from "@/components/flashcards/deck/deck.module.css";

import { getDatasetFor } from "@/data/flashcards/ai-sl/registry";

export default function DeckPageAISL() {
  const sp = useSearchParams();
  const topic = sp.get("topic") ?? "";
  const subtopic = sp.get("subtopic") ?? "";

  // --- infer audiența: mai întâi din URL, altfel din referrer (teacher/student)
  ...
  useEffect(() => {
    ...
    if (typeof document !== "undefined") {
      ...
      if (/\/AI_SL(?:\?|$).*aud=teacher/.test(ref) || /\/AI_SL\/flashcards\?aud=teacher/.test(ref)) {
        setAud("teacher");
      } else if (/\/AI_SL(?:\?|$).*aud=student/.test(ref) || /\/AI_SL\/flashcards\?aud=student/.test(ref)) {
        setAud("student");
      }
    }
  }, [sp]);

  // links care păstrează aud când există
  const aaLink = aud ? `/AI_SL?aud=${aud}` : "/AI_SL";
  const fcLink = aud ? `/AI_SL/flashcards?aud=${aud}` : "/AI_SL/flashcards";
  ...
  const goPrev = () => {
    if (endOfDeck) { if (total > 0) resetForNext(total - 1); return; }
  ...
  function viewSolution() { if (!endOfDeck) setMode("split"); }
  function cont() {
    if (idx < total - 1) resetForNext(idx + 1);
    else { setIdx(total); setMode("question"); }
  }
  function choose(id) {
    if (endOfDeck || ans.graded) return;
    setAnswers((prev) => { const next = [...prev]; next[idx] = { ...ans, selectedId: id }; return next; });
  }
  function restart() {
    setIdx(0); setMode("question"); setAttempted(0); setCorrect(0);
    setAnswers(Array.from({ length: total }, () => ({ selectedId: null, graded: false, isCorrect: false })));
  }
  ...
  return (
    <main
      className="course-theme-ai-sl mx-auto max-w-7xl px-4 pt-3 pb-6"
      style={{ "--breadcrumbs-h": "44px" }}
    >
      {/* Breadcrumb cu linkuri maro (fixăm culoarea pe toate stările) */}
          ...
          <li>
            <Link
              ...
              className="hover:underline underline-offset-2
                         !text-[var(--ai-sl-dark)]
                         hover:!text-[var(--ai-sl-dark)]
                         focus:!text-[var(--ai-sl-dark)]
                         active:!text-[var(--ai-sl-dark)]
                         visited:!text-[var(--ai-sl-dark)]"
            >
              Home
            </Link>
          </li>
      ...
      <section className={styles.wrap}>
        ...
        <div className={styles.leftCol}>
          {!endOfDeck ? (
            ...
            <div className={styles.endCard}>
              ...
              <p className={styles.endSubtitle}>
                AI SL — {topicLabel} — {subtopicLabel}
              </p>
            </div>
          )}
          ...
          <div className={styles.bottomNav}>
            <button className={styles.navArrow} onClick={goPrev} disabled={idx === 0} aria-label="Previous">←</button>
            <span className={styles.navCounter}>{Math.min(idx + 1, total)} / {total}</span>
            <button className={styles.navArrow} onClick={goNext} disabled={endOfDeck} aria-label="Next">→</button>
          </div>
        </div>
