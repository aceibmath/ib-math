...
export async function GET() {
  try {
    ...
    // Stripe search după metadata.firebase_uid
    try {
    ...
    // 3) Invoices (expand valid, fără nivele >4)
    const aggregate = [];
    for (const cid of ids) {
      const list = await stripe.invoices.list({
        ...
        expand: [
          ...
          // NU extindem product-ul aici (prea adânc) – îl luăm separat din Price
        ],
      });
      ...
      for (const inv of list.data || []) {
        ...
        aggregate.push({
          ...
          priceId,
          // product_id nu e garantat fără expand; îl putem aduce dacă chiar e necesar
          product_id:
