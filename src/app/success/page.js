...
export default async function SuccessPage({ searchParams }) {
  // în Next 15, searchParams e AsyncIterable în RSC – îl "așteptăm"
  ...
  return (
    <main style={{ maxWidth: 680, margin: "40px auto", padding: 16, textAlign: "center" }}>
      <h1>Payment Successful ✅</h1>
      <p>Thank you! Your payment has been processed successfully.</p>

      {ref ? <p style={{ fontSize: 14, opacity: 0.8 }}>Reference: {ref}</p> : null}
      {sessionId ? <p style={{ fontSize: 14, opacity: 0.8 }}>Session: {sessionId}</p> : null}
