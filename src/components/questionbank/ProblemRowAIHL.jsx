// src/components/questionbank/ProblemRowAIHL.jsx
"use client";

import { useEffect, useState } from "react";
import MdMath from "@/components/md/MdMath";
import styles from "@/app/(content)/AI_HL/questionbank/questionbank-aihl.module.css";
import { useAuth } from "../../context/AuthContext";

export default function ProblemRowAIHL({
  p: pProp,
  problem,
  item,
  index = 0,
  ...
  // salvează numele userului ca să îl putem trimite la Tutor IA
  useEffect(() => {
  ...
  // 0 = doar problem statement; 1 = split; 2 = full MS
  const [mode, setMode] = useState(0);

  // ascundem scrollbar global când intrăm în split/full MS
  useEffect(() => {
  ...
  // dacă deschizi Mark Scheme la altă întrebare => închide-o aici
  useEffect(() => {
    const handler = (e) => {
  ...
  /** Control CALCULATOR chip (ascunde dacă nu avem informație clară)
   * Acceptă:
  ...
  // Flag-uri opționale pe problemă (implicit true dacă lipsesc)
  const want = {
      ...
      >
        ...
        <div className={styles.cardHeader}>
          ...
          <div className={styles.headerRight}>
            {/* CALC chip: numai dacă avem etichetă + nu e ascuns */}
