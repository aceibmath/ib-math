...
const legacyProjectId = process.env.FIREBASE_ADMIN_PROJECT_ID || process.env.GOOGLE_CLOUD_PROJECT;
const legacyClientEmail = process.env.FIREBASE_ADMIN_CLIENT_EMAIL;
const legacyPrivateKey = normalizePrivateKey(process.env.FIREBASE_ADMIN_PRIVATE_KEY || "");
...
function loadServiceAccount() {
  ...
  if (process.env.FIREBASE_SERVICE_ACCOUNT_JSON) {
    try {
      ...
      console.error("[firebaseAdmin] FIREBASE_SERVICE_ACCOUNT_JSON parse error:", e?.message);
    }
  }

  // c) JSON Base64 (Vercel)
  if (process.env.FIREBASE_SERVICE_ACCOUNT_BASE64) {
    try {
      ...
      console.error("[firebaseAdmin] FIREBASE_SERVICE_ACCOUNT_BASE64 decode/parse error:", e?.message);
    }
  }

  // d) Cale locală spre fișier (Windows dev)
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    try {
      ...
      console.error("[firebaseAdmin] Failed to read GOOGLE_APPLICATION_CREDENTIALS file:", e?.message);
    }
  }
